...
async function showUsersMenu(userId, messageId) {
  ...
  for (let i = 0; i < users.length; i++) {
    const u = users[i];
    const status = u.is_banned ? 'BANNED' : (u.is_restricted ? 'RESTRICTED' : 'OK');
    lines.push(status + ' ' + u.full_name + ' | Rs ' + (u.balance || 0).toFixed(0));
    keyboard.push([
      { text: u.full_name, callback_data: 'adm_user_' + u.user_id }
    ]);
  }
  keyboard.push([{ text: 'Back', callback_data: 'adm_menu' }]);
...
async function showUserDetail(userId, targetUid) {
  const database = db.getDB();
  const u = await database.get('SELECT * FROM users WHERE user_id = ?', targetUid);
  ...
  lines.push('');
  lines.push('Name: ' + u.full_name);
  lines.push('ID: ' + u.user_id);
  lines.push('Balance: Rs ' + (u.balance || 0).toFixed(0));
  lines.push('Status: ' + (u.is_banned ? 'BANNED' : (u.is_restricted ? 'RESTRICTED' : 'Active')));
  lines.push('Joined: ' + u.created_at);
  ...
  const keyboard = {
    inline_keyboard: [
      [
        ...
        { text: u.is_restricted ? 'Unrestrict' : 'Restrict', callback_data: 'adm_restrict_' + targetUid }
      ],
      [{ text: 'Send Message', callback_data: 'adm_msg_' + targetUid }],
      [{ text: 'Back', callback_data: 'adm_users' }]
    ]
  };
...
async function toggleBanUser(userId, targetUid) {
  ...
  await database.run('UPDATE users SET is_banned = ? WHERE user_id = ?', newVal, targetUid);
  await bot.sendMessage(userId, newVal ? 'User banned.' : 'User unbanned.', { reply_markup: adminBackKeyboard() });
}
...
async function toggleRestrictUser(userId, targetUid) {
  ...
  const u = await database.get('SELECT * FROM users WHERE user_id = ?', targetUid);
  if (!u) { await bot.sendMessage(userId, 'User not found.'); return; }
  const newVal = u.is_restricted ? 0 : 1;
  await database.run('UPDATE users SET is_restricted = ? WHERE user_id = ?', newVal, targetUid);
  await bot.sendMessage(userId, newVal ? 'User restricted.' : 'User unrestricted.', { reply_markup: adminBackKeyboard() });
}

async function showStats(userId, messageId) {
  const database = db.getDB();
  const totalUsers = await database.get('SELECT COUNT(*) as count FROM users');
  const totalDeposits = await database.get("SELECT SUM(amount_inr) as total FROM deposits WHERE status = 'approved'");
  const totalOrders = await database.get('SELECT COUNT(*) as count FROM orders');
  const pendingDeposits = await database.get("SELECT COUNT(*) as count FROM deposits WHERE status = 'pending'");

  const lines = [];
  lines.push('Store Statistics');
  lines.push('');
  lines.push('Total Users: ' + (totalUsers.count || 0));
  lines.push('Total Orders: ' + (totalOrders.count || 0));
  lines.push('Pending Deposits: ' + (pendingDeposits.count || 0));
  lines.push('Total Revenue: Rs ' + ((totalDeposits.total || 0)).toFixed(0));
...
async function startBroadcast(userId, messageId) {
  const state = getState(userId);
  state.waitingBroadcast = true;
  await bot.editMessageText('Broadcast. Enter the message to send to all users:', {
    chat_id: userId,
    message_id: messageId,
    reply_markup: adminBackKeyboard()
  });
...
async function showPasswordSettings(userId, messageId) {
  const keyboard = {
    inline_keyboard: [
      [{ text: 'Change Password', callback_data: 'adm_change_password' }],
      [{ text: 'Back', callback_data: 'adm_menu' }]
    ]
  };
  await bot.editMessageText('Password Settings. Change the admin panel password.', {
    chat_id: userId,
    message_id: messageId,
    reply_markup: keyboard
  });
...
bot.on('message', async (msg) => {
  ...
  if (msg.photo && state.waitingQrUpload) {
    state.waitingQrUpload = false;
    const method = state.qrUploadMethod;
    state.qrUploadMethod = null;
    const photo = msg.photo[msg.photo.length - 1];
    const fileId = photo.file_id;
    const keyMap = { gpay: 'qr_gpay', fampay: 'qr_fampay', any: 'qr_any', bnb: 'qr_bnb' };
    await db.setSetting(keyMap[method] || ('qr_' + method), fileId);
    await bot.sendMessage(userId, 'QR code updated!', { reply_markup: adminBackKeyboard() });
    return;
  }
  ...
  if (state.waitingPassword) {
    state.waitingPassword = false;
    if (text === currentPassword) {
      authenticatedUsers.add(userId);
      await bot.sendMessage(userId, 'Authenticated successfully!');
      await showAdminMenu(userId);
    } else {
      await bot.sendMessage(userId, 'Wrong password. Try /start again.');
    }
    return;
  }

  if (state.waitingNewPassword) {
    state.waitingNewPassword = false;
    currentPassword = text;
    await bot.sendMessage(userId, 'Password changed!', { reply_markup: adminBackKeyboard() });
    return;
  }

  if (state.waitingUpiId) {
    state.waitingUpiId = false;
    await db.setSetting('upi_id', text);
    await bot.sendMessage(userId, 'UPI ID updated: ' + text, { reply_markup: adminBackKeyboard() });
    return;
  }

  if (state.waitingRate) {
    state.waitingRate = false;
    const rate = parseFloat(text);
    if (isNaN(rate) || rate <= 0) { await bot.sendMessage(userId, 'Invalid rate.'); return; }
    await db.setSetting('usdt_to_inr_rate', text);
    await bot.sendMessage(userId, '1 USDT = Rs ' + text, { reply_markup: adminBackKeyboard() });
    return;
  }

  if (state.waitingRefReward) {
    state.waitingRefReward = false;
    const reward = parseFloat(text);
    if (isNaN(reward) || reward < 0) { await bot.sendMessage(userId, 'Invalid amount.'); return; }
    await db.setSetting('referral_reward_inr', text);
    await bot.sendMessage(userId, 'Referral reward: Rs ' + text, { reply_markup: adminBackKeyboard() });
    return;
  }

  if (state.waitingGroupLink) {
    state.waitingGroupLink = false;
    await db.setSetting('group_link', text);
    await bot.sendMessage(userId, 'Group link updated!', { reply_markup: adminBackKeyboard() });
    return;
  }

  if (state.waitingChannelLink) {
    state.waitingChannelLink = false;
    await db.setSetting('channel_link', text);
    await bot.sendMessage(userId, 'Channel link updated!', { reply_markup: adminBackKeyboard() });
    return;
  }

  if (state.waitingSupportLink) {
    state.waitingSupportLink = false;
    await db.setSetting('support_link', text);
    await bot.sendMessage(userId, 'Support link updated!', { reply_markup: adminBackKeyboard() });
    return;
  }

  if (state.waitingSendMsg) {
    state.waitingSendMsg = false;
    const targetId = state.sendMsgTo;
    state.sendMsgTo = null;
    await notifyUserViaUserBot(targetId, text);
    await bot.sendMessage(userId, 'Message sent to ' + targetId, { reply_markup: adminBackKeyboard() });
    return;
  }

  if (state.waitingBroadcast) {
    state.waitingBroadcast = false;
    const users = await db.getAllUsers();
    let sent = 0, failed = 0;
    for (let i = 0; i < users.length; i++) {
      try { await notifyUserViaUserBot(users[i].user_id, text); sent++; }
      catch (e) { failed++; }
    }
    await bot.sendMessage(userId, 'Broadcast done! Sent: ' + sent + ' Failed: ' + failed, { reply_markup: adminBackKeyboard() });
    return;
  }

  if (state.waitingProductEdit) {
    state.waitingProductEdit = false;
    const pid = state.editingProductId;
    const field = state.editingField;
    state.editingProductId = null;
    state.editingField = null;
    const database = db.getDB();
    let value = text;
    if (field === 'price_inr' || field === 'stock') {
      value = parseFloat(text);
      if (isNaN(value)) { await bot.sendMessage(userId, 'Invalid number.'); return; }
    }
    const allowed = ['name', 'price_inr', 'stock', 'description', 'content'];
    if (!allowed.includes(field)) { await bot.sendMessage(userId, 'Invalid field.'); return; }
    await database.run('UPDATE products SET ' + field + ' = ? WHERE id = ?', value, pid);
    await bot.sendMessage(userId, 'Product updated!', { reply_markup: adminBackKeyboard() });
    return;
  }
  ...
  if (state.addingProduct) {
    ...
    if (ap.step === 'name') {
      ap.name = text; ap.step = 'price';
      await bot.sendMessage(userId, 'Step 2/5: Enter price in Rs:');
    } else if (ap.step === 'price') {
      const price = parseFloat(text);
      if (isNaN(price) || price <= 0) { await bot.sendMessage(userId, 'Invalid price:'); return; }
      ap.price_inr = price; ap.step = 'stock';
      await bot.sendMessage(userId, 'Step 3/5: Enter stock count:');
    } else if (ap.step === 'stock') {
      const stock = parseInt(text);
      if (isNaN(stock) || stock < 0) { await bot.sendMessage(userId, 'Invalid stock:'); return; }
      ap.stock = stock; ap.step = 'description';
      await bot.sendMessage(userId, 'Step 4/5: Enter description:');
    } else if (ap.step === 'description') {
      ap.description = text; ap.step = 'content';
      await bot.sendMessage(userId, 'Step 5/5: Enter product content:');
    } else if (ap.step === 'content') {
      ap.content = text;
      state.addingProduct = null;
      const database = db.getDB();
      await database.run(
        'INSERT INTO products (name, price_inr, stock, description, content, is_active) VALUES (?, ?, ?, ?, ?, 1)',
        ap.name, ap.price_inr, ap.stock, ap.description, ap.content
      );
      await bot.sendMessage(userId, 'Product added!', { reply_markup: adminBackKeyboard() });
    }
    return;
  }
});
