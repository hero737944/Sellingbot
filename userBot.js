...
async function showSupport(userId, messageId) {
  ...
  lines.push('');
  lines.push('⚠️ All purchases made are final - no refunds or replacements will be provided under any circumstances, and all products are bought entirely at the buyer own risk.');
...
async function handleDepositAmount(userId, text) {
  ...
  const qrKeyMap = { gpay: 'qr_gpay', fampay: 'qr_fampay', any: 'qr_any', bnb: 'qr_bnb' };
  ...
  lines.push('👉 Pay exactly Rs ' + amount.toFixed(0) + ' (⚠️ Do Not Change The Amount)');
  lines.push("🟢 After Successful Payment Click 'I Have Paid' below");
  lines.push('');
  lines.push('⚠️ The QR Code Is Only Valid For 15 Minutes!');
  ...
  if (qrFileId) {
    ...
    await bot.sendMessage(userId, fallbackText, { reply_markup: keyboard });
  }
}
...
async function handleScreenshot(userId, photos) {
  ...
  await database.run('UPDATE deposits SET screenshot_file_id = ? WHERE ref_id = ?', fileId, refId);
  ...
  await bot.sendMessage(userId, waitText, { reply_markup: utils.getBackKeyboard() });
  ...
  if (deposit) {
    ...
    const adminKeyboard = {
      inline_keyboard: [
        ...
        [{ text: '✉️ Send Message', callback_data: 'adm_msg_' + userId }]
      ]
    };
